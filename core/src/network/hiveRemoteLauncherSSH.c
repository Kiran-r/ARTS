...
#include "hiveGlobals.h"         //For hiveGloablMessageTable
#include "hiveMalloc.h"          //For hiveMalloc, hiveFree
#include "hiveRemoteLauncher.h"  //For struct hiveLauncher
...
void hiveRemoteLauncherSSHStartupProcesses( struct hiveRemoteLauncher * launcher )
{
    ...
    struct hiveConfig * config = launcher->config;
    ...
    sshExecutions = hiveMalloc( sizeof( FILE *  ) * config->tableLength-1 );
    ...
    for(k=startNode+1; k< config->tableLength+startNode; k++ )
    {
        ...
        if(child == 0)
        {
            ...
            //execlp("ssh", "-f", config->table[i].ipAddress, "cd runLocal; /home/land350/intel/test/intel/inspector_xe_2016.1.1.435552/bin64/inspxe-cl -c mi3 /home/land350/new/dtcp/test/hiveFib 20", (char *)NULL); 
        }
    }
    if(killMode)
        exit(0);
