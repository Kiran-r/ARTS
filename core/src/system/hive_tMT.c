...
//    if(init)
    ...
    if(flipstate) 
    {
        ...
        uint64_t res = artsAtomicFetchXOrU64(all_states, current_pos);
//        if(res > (1UL << 11))
//            PRINTF("BIG RES: %lu TID: %u\n", res, start);
    }
...
static inline void CreateContexts(ti_t* threadpool_info) 
{
    ...
    { // create alias thread (ATs) pool and potentially BT
        ...
        DPRINTF("Master %lu MThread 0x%x: Waiting in thread creation %d\n",
                threadpool_info->unit->id, pthread_self(), i);
...
static inline ticket_t _hiveCreateFuture(uint32_t paramc, uint64_t* paramv) // first parameter is pointer to future; last chooses DB creation
{
    ...
    DPRINTF("UNIT:MT/AT %lu:%lu Thread 0x%x Created Future....\n",
            threadUNIT(), threadID(), pthread_self());
...
void hiveGetFutures(ticket_t* ticket, unsigned int num) 
{
    ...
    DPRINTF("UNIT:MT/AT %lu:%lu Thread 0x%x Getting Futures....\n",
       threadUNIT(), threadID(), pthread_self());

    unsigned int cand = 0;
    while(!hiveTestStateOneLeft(threadpool_info->alias_running));
    while(!(cand = hiveNextCandidate(threadpool_info->alias_avail))); // WARN: if multi-entry, potential race condition

    DPRINTF("b) Unit %lu: Candidate %d Alias_Running 0x%x\n", unitID(), cand, *threadpool_info->alias_running);
