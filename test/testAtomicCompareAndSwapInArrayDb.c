...
{   
    ...
    if(!workerId && !nodeId)
    {
        ...
        hiveGuid_t endGuid = hiveEdtCreate(end, 0, 0, NULL, (numAdd+1)*elementsPerBlock*blocks + 1);
        ...
        for(unsigned int j=0; j<numAdd; j++)
        {
            for(unsigned int i=0; i<elementsPerBlock*blocks; i++)
            {
                ...
                hiveAtomicCompareAndSwapInArrayDb(array, i, j, j+1, endGuid, j*elementsPerBlock*blocks + i);
            }
        }
        ...
        for(unsigned int i=0; i<elementsPerBlock*blocks; i++)
        {
            ...
            hiveAtomicCompareAndSwapInArrayDb(array, i, numAdd+1, 0, endGuid, numAdd*elementsPerBlock*blocks + i);
        }
    }
}
